...
const PLANS: Plan[] = [
  {
    ...
    price: "0",
    period: "7 ngày",
    description: "Làm quen với AI chấm điểm và kiểm tra trình độ CEFR ban đầu hoàn toàn miễn phí.",
    features: [
  ...
  },
  {
    ...
    price: "129.000",
    period: "tháng",
    description: "Lộ trình lý tưởng cho người bận rộn muốn duy trì thói quen nói tiếng Anh hằng tuần.",
    features: [
  ...
  },
  {
    ...
    price: "249.000",
    period: "tháng",
    description: "Giải pháp bứt phá tối ưu cho người luyện thi IELTS, chuẩn bị phỏng vấn xin việc.",
    features: [
      ...
      "Đầy đủ gợi ý nâng band từ vựng C1/C2",
      "Nghe phát âm chuẩn giọng bản xứ (TTS)",
      "Báo cáo tiến độ trực quan theo tuần"
    ],
    isPopular: true,
    buttonText: "Chinh Phục Ngay",
    accent: "border-purple-500/40 bg-purple-500/10 shadow-2xl backdrop-blur-md shadow-purple-500/5"
  },
  {
    id: "unlimited",
    name: "Unlimited",
    price: "499.000",
    period: "tháng",
    description: "Luyện nói thả ga không giới hạn dành cho người khao khát làm chủ tiếng Anh trong thời gian ngắn nhất.",
    features: [
...
export default function Pricing({ onPlanSelect }: PricingProps) {
  ...
  return (
    <section id="pricing-section" className="py-20 bg-[#030014] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-10">
          <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight mb-4">
            Đầu Tư Cho <span className="gradient-text bg-gradient-to-r from-blue-400 to-purple-400">Giọng Nói</span> Của Bạn
          </h2>
          <p className="text-gray-400 text-sm sm:text-base max-w-xl mx-auto">
            Không có hợp đồng ràng buộc. Nâng cấp hoặc hủy gói bất kỳ lúc nào bạn muốn. Chọn gói dịch vụ phù hợp nhất với tần suất luyện nói của bạn.
          </p>
        </div>

        {/* Annual / Monthly Toggle */}
        <div className="flex items-center justify-center gap-3.5 mb-16">
          <span className={`text-sm font-semibold transition-all ${!isYearly ? "text-white" : "text-gray-500"}`}>
          ...
          >
            <div className={`w-4.5 h-4.5 rounded-full bg-purple-500 transition-all ${isYearly ? "translate-x-5.5 bg-blue-400" : "translate-x-0"}`} />
          </button>
          <span className={`text-sm font-semibold transition-all flex items-center gap-1.5 ${isYearly ? "text-white" : "text-gray-500"}`}>
            Thanh toán hằng năm
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
          {PLANS.map((plan) => {
            ...
            const finalPrice = isYearly && basePrice > 0 
            ...
            return (
              ...
              >
                  ...
                  <div className="space-y-3">
                    {plan.features.map((feature, idx) => (
                      <div key={idx} className="flex items-start gap-2.5">
                        <Check className={`w-4 h-4 shrink-0 mt-0.5 rounded-full p-0.5 ${
                          plan.isPopular ? "bg-purple-500/15 text-purple-300" : "bg-white/5 text-gray-400"
                        }`} />
                ...
                >
                  {plan.isPopular && <Sparkles className="w-3.5 h-3.5 text-purple-200" />}
                  {plan.buttonText}
                </button>
              </div>
            );
